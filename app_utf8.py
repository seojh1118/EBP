...
GUIDE_FILE = "guide_voice.mp3" # 안내 음성 파일명

# [안전장치] 절대 죽지 않는 영생 단어 목록 (API 실패/타임아웃 방지용)
IMMORTAL_WORDS = [
...
# --- 1. 상태 저장 함수 (빔프로젝터 통신) ---
def update_projector(color, text, status="active"):
    state = {
        "status": status,
        "color": color,
        "text": text,
...
def check_is_standard_word(word):
    # [1차 필터] 하드코딩된 영생 단어 (네트워크 오류 방지)
    if word in IMMORTAL_WORDS:
        return True
...
# --- 4. 시뮬레이션 데이터 생성 (비상용) ---
def generate_simulation_data(word):
...
def on_stt_button_click():
    ...
    try:
        ...
        with sr.Microphone() as source:
            ...
            update_projector("#FFFF00", "청취 중...", "listening") # 빔: 한글
...
def main():
    ...
    if "text" in st.session_state and st.session_state.text:
        ...
        update_projector("#9900FF", "분석 중...", "analyzing") # 빔: 한글
        ...
        if any(bw in text for bw in bad_words):
            st.error("🚫 비속어 감지됨")
            update_projector("#FF0000", "비속어", "result") # 빔: 한글
            st.stop()

        # 2. 표준어 확인 (안전장치 포함)
        if check_is_standard_word(text):
            st.success(f"♾️ 영생 (표준어): {text}")
            update_projector("#FFFFFF", text, "result")
            if st.button("초기화"): 
                update_projector("#000000", "", "standby")
        ...
        if text in KNOWN_SLANGS:
            ...
            _, series = generate_simulation_data(text) 
            is_sim = True
            
        # 4. 실시간 분석 + AI 예측
        else:
            with st.spinner("AI가 유행 패턴을 분석 중입니다..."):
                assets = load_assets()
                features, series, is_sim = get_realtime_features(text)
                
                # KNN 모델 사용
                if assets and assets[0]:
                    knn_model, scaler = assets
                    months = int(knn_model.predict(scaler.transform([features]))[0])
                    reason = "AI 모델 예측 결과"
                else:
                    # 모델 없으면 해시 랜덤 (전시 펑크 방지)
        ...
        update_projector(color, status_msg, "result")
